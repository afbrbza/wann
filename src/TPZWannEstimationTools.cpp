...
void TPZWannEstimationTools::hRefinement(TPZGeoMesh *gmesh, TPZVec<int64_t> &needRefinement, ProblemData *SimData) {
  ...
  std::set<int> matIds = {
      SimData->ESurfWellCyl, SimData->EPressure2DSkin, SimData->EPressureInterface, SimData->EHDivBoundInterface};
...
void TPZWannEstimationTools::FakeRefine2(TPZGeoMesh *gmesh, ProblemData *SimData) {
  std::set<int> matIds = {
      SimData->ESurfWellCyl, SimData->EPressure2DSkin, SimData->EPressureInterface, SimData->EHDivBoundInterface, SimData->EFarField, SimData->ESurfHeel, SimData->ESurfToe, SimData->ECapRock};
...
TPZVec<int64_t> TPZWannEstimationTools::ErrorEstimationOld(TPZMultiphysicsCompMesh *cmeshMixed, TPZCompMesh *cmesh, ProblemData *SimData) {
  ...
  for (int64_t icel = 0; icel < ncel; ++icel) {
    ...
    elementErrors[icel] = pow((sqrt(fluxError) + (hk / M_PI) * sqrt(balanceError)), 2) + conformityError;
