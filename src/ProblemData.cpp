...
// readjson function. takes a json function as parameter and completes the
...
void ProblemData::ReadJson(std::string file) {
    ...
    json input = json::parse(
        filejson, nullptr, true, true); // to ignore comments in json file

    if (input.find("MeshData") == input.end()) DebugStop();
    json meshdata = input["MeshData"];
    if (meshdata.find("file") == meshdata.end()) DebugStop();
    m_Mesh.file = meshdata["file"];
    if (meshdata.find("NumUniformRef") == meshdata.end()) DebugStop();
    m_Mesh.NumUniformRef = meshdata["NumUniformRef"];
    if (meshdata.find("NumDirRef") == meshdata.end()) DebugStop();
    m_Mesh.NumDirRef = meshdata["NumDirRef"];
    if (meshdata.find("ToCylindrical") == meshdata.end()) DebugStop();
    ...
    if (wellbore.find("eccentricity") == wellbore.end()) DebugStop();
    for (int i = 0; i < 3; i++) {
        if (wellbore["eccentricity"][i].is_null()) DebugStop();
    ...
    if (reservoir.find("porosity") == reservoir.end()) DebugStop();
    ...
    if (input.find("FluidData") == input.end()) DebugStop();
    json fluid = input["FluidData"];
    if (fluid.find("name") == fluid.end()) DebugStop();
    m_Fluid.name = fluid["name"];
    if (fluid.find("viscosity") == fluid.end()) DebugStop();
    m_Fluid.viscosity = fluid["viscosity"];
    if (fluid.find("density") == fluid.end()) DebugStop();
    m_Fluid.density = fluid["density"];

    if (input.find("PostProcData") == input.end()) DebugStop();
    json postproc = input["PostProcData"];
    if (postproc.find("wellbore_vtk") == postproc.end()) DebugStop();
    m_PostProc.wellbore_vtk = postproc["wellbore_vtk"];
    if (postproc.find("reservoir_vtk") == postproc.end()) DebugStop();
    m_PostProc.reservoir_vtk = postproc["reservoir_vtk"];
    if (postproc.find("training_data") == postproc.end()) DebugStop();
    m_PostProc.training_data = postproc["training_data"];
    if (postproc.find("vtk_resolution") == postproc.end()) DebugStop();
    m_PostProc.vtk_resolution = postproc["vtk_resolution"];
    if (postproc.find("training_resolution") == postproc.end()) DebugStop();
...
void ProblemData::Print(std::ostream &out) {
    ...
    out << "Number of uniform refinements: " << m_Mesh.NumUniformRef
        << std::endl;
    out << "Number of directional refinements: " << m_Mesh.NumDirRef
        << std::endl;
    out << "Cylindrical map: " << (m_Mesh.ToCylindrical ? "Yes" : "No")
        << std::endl
        << std::endl;
    ...
    out << "Training Data Resolution: " << m_PostProc.training_resolution
        << std::endl;
